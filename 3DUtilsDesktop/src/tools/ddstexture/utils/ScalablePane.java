...
public class ScalablePane extends JPanel {
	...
	public ScalablePane(Image master, boolean toFit, boolean highQuality) {
		this.master = master;
		setToFit(toFit);
	...
	public Image getScaledInstanceToFit(Image master, Dimension size) {
		Dimension masterSize = new Dimension(master.getWidth(this), master.getHeight(this));
		return getScaledInstance(toBufferedImage(master), getScaleFactorToFit(masterSize, size), highQuality ? 
				RenderingHints.VALUE_INTERPOLATION_BILINEAR : RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR, highQuality);
	}

	public Image getScaledInstanceToFill(Image master, Dimension size) {
		Dimension masterSize = new Dimension(master.getWidth(this), master.getHeight(this));
		return getScaledInstance(toBufferedImage(master), getScaleFactorToFill(masterSize, size), highQuality ? 
				RenderingHints.VALUE_INTERPOLATION_BILINEAR : RenderingHints.VALUE_INTERPOLATION_NEAREST_NEIGHBOR, highQuality);
	}
	...
	protected BufferedImage getScaledDownInstance(	BufferedImage img, int targetWidth, int targetHeight, Object hint,
		...
		int type = (img
				.getTransparency() == Transparency.OPAQUE) ? BufferedImage.TYPE_INT_RGB : BufferedImage.TYPE_INT_ARGB;
