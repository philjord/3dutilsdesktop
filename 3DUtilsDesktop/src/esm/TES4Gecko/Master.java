...
public class Master extends SerializedElement {
	...
	public PluginRecord getRecord(int formID) throws DataFormatException, IOException, PluginException {
		...
		if (formInfo == null) {
			throw new PluginException(String.format("%s: Record %08X not found",
					new Object[] {this.masterFile.getName(), Integer.valueOf(masterFormID)}));
		}
		...
		try {
			...
			if (count != 20) {
				throw new PluginException(String.format("%s: Record %08X truncated",
						new Object[] {this.masterFile.getName(), Integer.valueOf(masterFormID)}));
			}
