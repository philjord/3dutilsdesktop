...
public class Plugin extends SerializedElement {
	...
	private static final String[]		initialGroupList		= {"GMST", "GLOB", "CLAS", "FACT", "HAIR", "EYES",
		"RACE", "SOUN", "SKIL", "MGEF", "SCPT", "LTEX", "ENCH", "SPEL", "BSGN", "ACTI", "APPA", "ARMO", "BOOK", "CLOT",
		"CONT", "DOOR", "INGR", "LIGH", "MISC", "STAT", "GRAS", "TREE", "FLOR", "FURN", "WEAP", "AMMO", "NPC_", "CREA",
		"LVLC", "SLGM", "KEYM", "ALCH", "SBSP", "SGST", "LVLI", "WTHR", "CLMT", "REGN", "CELL", "WRLD", "DIAL", "QUST",
		...
		if (parentGroup == null) {
			if ((record instanceof PluginGroup)) {
				throw new PluginException(String.format("Type %d group does not have a parent",
						new Object[] {Integer.valueOf(((PluginGroup)record).getGroupType())}));
			}
			throw new PluginException(String.format("%s record %s (%08X) does not have a parent",
					new Object[] {record.getRecordType(), record.getEditorID(), Integer.valueOf(record.getFormID())}));
		}
			...
			if ((groupType == 1) || (groupType == 6) || (groupType == 7)) {
				...
				for (int index = 1; index < recordCount; index++) {
					if (recordList.get(index) == group) {
						...
						if ((!recordType.equals("WRLD"))	&& (!recordType.equals("CELL"))
							&& (!recordType.equals("DIAL")))
							break;
						int formID = prevRecord.getFormID();
						if ((group.getGroupParentID() != formID)
							|| (this.formMap.get(new Integer(groupParentID)) != null))
							break;
		...
		if (index >= 0) {
			...
			if (((recordType.equals("WRLD")) || (recordType.equals("CELL")) || (recordType.equals("DIAL")))
				...
				if ((checkRecord instanceof PluginGroup)) {
					...
					if (((groupType == 1) || (groupType == 6) || (groupType == 7))
						&& (subgroup.getGroupParentID() == formID)) {
	...
	public String getCellRegionsUsedStr() {
		...
		for (Integer i : regionList) {
			if (retStr.equals(""))
				retStr = retStr + String.format("%08X", new Object[] {i});
			else
				retStr = retStr + "-" + String.format("%08X", new Object[] {i});
		}
		return retStr;
							...
							if (cell.getRecordType().equals("CELL")) {
								...
								try {
									...
									if ((ESMToolMain.debugMode) && (XYLoc != null)) {
										...
										System.out.printf(
												"ignoreAllExteriorCellsExcept: Cell %08X does not have XY coordinates\n",
												new Object[] {Integer.valueOf(cell.getFormID())});
									}
									ignoreCell = true;
								}
							...
							if (cell.getRecordType().equals("CELL")) {
								...
								try {
									...
									if ((ESMToolMain.debugMode) && (XYLoc != null)) {
										...
										System.out.printf(
												"ignoreAllExteriorCells: Cell %08X does not have XY coordinates\n",
												new Object[] {Integer.valueOf(cell.getFormID())});
									}
									ignoreCell = true;
								}
	...
	public void load(WorkerTask task) throws PluginException, DataFormatException, IOException, InterruptedException {
		...
		try {
			...
			if (loadCount != recordCount) {
				String text = this.pluginFile.getName() + ": Load count " + loadCount + " does not match header count "
								+ recordCount;
				selection = WorkerDialog.showConfirmDialog(task.getParent(), text + ". Do you want to continue?",
						"Error", 0, 0, false);
				if (selection != 0) {
